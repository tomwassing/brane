...
async fn ensure_image(
    ...
    if let Some(image_file) = &exec.image_file {
        ...
        debug!(" > Pulling image '{}'...", exec.image);
...
/***** LIBRARY EXECUTOR *****/
/// The DockerExecutor, which executed external jobs in a local, non-instance setting.
...
impl DockerExecutor {
    /// Constructor for teh DockerExecutor.
    /// 
    /// **Arguments**
    ///  * `data`: If given, references the path that should be mounted under the JuiceFS filesystem.
    #[inline]
